...
export const VentasPremiosBolivaresEncargada = ({
  ...
  const parleySystemCodes = ['INMEJORABLE-MULTIS-1', 'INMEJORABLE-MULTIS-2', 'INMEJORABLE-MULTIS-3', 'INMEJORABLE-MULTIS-4', 'INMEJORABLE-5Y6', 'POLLA', 'MULTISPORT-CABALLOS-NAC', 'MULTISPORT-CABALLOS-INT', 'MULTISPORT-5Y6'];

  // Agrupar sistemas por sistema padre
  const groupSystemsByParent = (systemsList: typeof systems) => {
    const grouped = new Map<string, {
      parent: SystemEntry | null;
      ...
      if (system.parent_system_id) {
        ...
        if (hasChildren) {
          ...
          if (!grouped.has(system.lottery_system_id)) {
            grouped.set(system.lottery_system_id, {
              parent: system,
              children: []
            });
          }
  ...
  useEffect(() => {
    ...
    systems.forEach(system => {
      ...
      newInputValues[salesKey] = salesBs > 0 ? salesBs.toLocaleString('es-VE', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      }) : '';
      newInputValues[prizesKey] = prizesBs > 0 ? prizesBs.toLocaleString('es-VE', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2
      }) : '';
    });
  ...
  const calculateNormalTotals = () => {
    return normalSystems.reduce((acc, system) => ({
      sales_bs: acc.sales_bs + (system.sales_bs || 0),
      prizes_bs: acc.prizes_bs + (system.prizes_bs || 0)
    }), {
      sales_bs: 0,
  ...
  const calculateParleyTotals = () => {
    return parleySystems.reduce((acc, system) => ({
      sales_bs: acc.sales_bs + (system.sales_bs || 0),
      prizes_bs: acc.prizes_bs + (system.prizes_bs || 0)
    }), {
      sales_bs: 0,
  ...
  return <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {parleySystems.length > 0 && <div className="mt-6 pt-6 border-t space-y-4">
